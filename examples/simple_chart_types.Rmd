...
tmp$collection_date_end<-tmp$collection_date + sample(10:30,nrow(date_tmp),replace = TRUE)

tmp$collection_date_end<-sapply(as.character(date_tmp$collection_date_end),function(x){
   if(runif(1)>0.9)
     return(x)
    return(NA)
})
...
Since there's a lot going on, let's subset this to get a better view of what is going on
```{r}
```
...
```{r imageOne_one}
interior_img<-input_data(file = system.file("extdata", "random_interior_map.tiff", package = "gevitR"),dataType = "image")
```
...
```{r imageOne_two, eval=FALSE, include=FALSE}
...
load(file = system.file("extdata", "img_meta.rds", package = "gevitR"))
...
```{r}
gel_img<-input_data(file = system.file("extdata", "gel_image.tiff", package = "gevitR"),dataType = "image")
...
```{r}
load(file="../inst/extdata/gel_img_meta.rds")
